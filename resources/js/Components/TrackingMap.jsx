...
const calculateDistanceKm = (lat1, lon1, lat2, lon2) => {
    if (!lat2 || !lon2) return 0; const R = 6371; const dLat = (lat2 - lat1) * (Math.PI / 180); const dLon = (lon2 - lon1) * (Math.PI / 180);
    const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(lat1 * (Math.PI / 180)) * Math.cos(lat2 * (Math.PI / 180)) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
    return R * (2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)));
};
const calculateETA = (distanceKm, speed) => {
    if (distanceKm < 1) return "Llegó al destino"; if (speed <= 0) return "N/D (Detenido)"; 
    const totalHours = distanceKm / speed; const hours = Math.floor(totalHours); const minutes = Math.round((totalHours - hours) * 60);
    if (hours === 0) return `${minutes} min`; return `${hours}h ${minutes}m`;
};
...
// 🧠 FORMATEO DE ALERTA
const formatAlertForUI = (newAlert) => {
    ...
    const typeStr = newAlert.type || 'DESCONOCIDO'; let finalTypeHtml = typeStr.replace('_', ' ');

    if (['AVERIA', 'PANICO', 'AUXILIO'].includes(typeStr)) { bgColor = 'bg-red-50'; cardStyle = 'border-red-500 shadow-md shadow-red-500/20'; badgeStyle = 'bg-red-600 text-white border-red-700 animate-pulse'; finalTypeHtml = dangerIcon + typeStr.replace('_', ' '); } 
    else if (['SIN_COMBUSTIBLE', 'GPS_APAGADO'].includes(typeStr)) { bgColor = 'bg-orange-50'; cardStyle = 'border-orange-500 shadow-md shadow-orange-500/20'; badgeStyle = 'bg-orange-600 text-white border-orange-700 animate-pulse'; finalTypeHtml = dangerIcon + typeStr.replace('_', ' '); } 
    else if (typeStr === 'EXCESO_VELOCIDAD') { bgColor = 'bg-amber-50'; cardStyle = 'border-amber-300'; badgeStyle = 'bg-amber-100 text-amber-700 border-amber-300'; } 
    else if (['LLEGADA_DESTINO', 'SALIDA'].includes(typeStr)) { bgColor = 'bg-green-50'; cardStyle = 'border-green-200'; badgeStyle = 'bg-white text-green-600 border-green-200'; } 

    const dateObj = newAlert.created_at ? new Date(newAlert.created_at) : new Date();
    return {
        id: newAlert.id || Math.random(), typeHtml: finalTypeHtml, typeRaw: typeStr, message: newAlert.message || 'Sin mensaje',
...
export default function TrackingMap() {
    ...
    // 🛡️ NUEVO: Controlador para cancelar peticiones obsoletas
    ...
    const fetchAlerts = (pageNumber = 1, isReset = false) => {
        setIsLoading(true);
        ...
        axios.get('/api/alerts', {
            ...
            if (isReset) { 
                ...
                setLiveAlerts(prev => [...prev, ...fetchedAlerts]); 
            }
            ...
            if (axios.isCancel(err)) {
                console.log("🛑 Petición cancelada por el usuario (cambio de filtro rápido).");
            } else {
                console.error("❌ Error consumiendo API /api/alerts:", err); 
                setIsLoading(false); 
            }
        });
    };
                    ...
                    <div className="bg-slate-800 p-4 shadow-md shrink-0 border-b border-slate-700">
                        ...
                        <div className="mt-3 space-y-2">
                            ...
                            >
                                ...
                                {Object.values(fleet).map(v => (
                                    <option key={v.id} value={v.license_plate}>{v.license_plate} - {v.model}</option>
                                ))}
                            </select>
                            ...
                            liveAlerts.map((alert, index) => (
                                <div key={`${alert.id}-${index}`} className={`p-3 rounded-lg border-2 ${alert.bg} ${alert.cardStyle} transition-all hover:shadow-lg animate-fade-in-up flex flex-col gap-2 bg-white/90`}>
                                    <div className="flex justify-between items-start">
                                        <span className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-full border ${alert.badgeStyle}`} dangerouslySetInnerHTML={{ __html: alert.typeHtml }} />
