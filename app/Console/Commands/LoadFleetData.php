...
class LoadFleetData extends Command
{
    ...
    public function handle()
    {
        ...
        // V1: Formosa -> Clorinda (Normal)
        $t1 = Trip::create(['vehicle_id' => $v1->id, 'start_time' => Carbon::now(), 'status' => 'EN_RUTA', 'origin' => 'Formosa', 'destination' => 'Clorinda', 'destination_lat' => $clorindaLat, 'destination_lng' => $clorindaLng]);
        Location::create(['trip_id' => $t1->id, 'latitude' => $formosaLat, 'longitude' => $formosaLng, 'speed' => 0, 'fuel_level' => 100, 'is_stopped' => true]);

        // V2: Formosa -> Pirané (Normal)
        $t2 = Trip::create(['vehicle_id' => $v2->id, 'start_time' => Carbon::now(), 'status' => 'EN_RUTA', 'origin' => 'Formosa', 'destination' => 'Pirané', 'destination_lat' => $piraneLat, 'destination_lng' => $piraneLng]);
        Location::create(['trip_id' => $t2->id, 'latitude' => $formosaLat, 'longitude' => $formosaLng, 'speed' => 0, 'fuel_level' => 100, 'is_stopped' => true]);

        // V3: El Colorado -> Formosa (Se averiará a mitad de camino)
        $t3 = Trip::create(['vehicle_id' => $v3->id, 'start_time' => Carbon::now(), 'status' => 'EN_RUTA', 'origin' => 'El Colorado', 'destination' => 'Formosa', 'destination_lat' => $formosaLat, 'destination_lng' => $formosaLng]);
        Location::create(['trip_id' => $t3->id, 'latitude' => $coloradoLat, 'longitude' => $coloradoLng, 'speed' => 0, 'fuel_level' => 100, 'is_stopped' => true]);

        // V4: Clorinda -> Formosa (Se quedará sin nafta - Arranca con solo 20%)
        $t4 = Trip::create(['vehicle_id' => $v4->id, 'start_time' => Carbon::now(), 'status' => 'EN_RUTA', 'origin' => 'Clorinda', 'destination' => 'Formosa', 'destination_lat' => $formosaLat, 'destination_lng' => $formosaLng]);
